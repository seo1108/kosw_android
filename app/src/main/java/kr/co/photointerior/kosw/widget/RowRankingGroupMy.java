 ...
 * 개인 랭킹 나의 순위
 */
public class RowRankingGroupMy extends LinearLayout {
    private LinearLayout mBox;
    private TextView mRanking;
    private TextView mDepart;
    private TextView mAmount;

    public RowRankingGroupMy(Context context) {
        super(context);
        initView(context);
    }

    public RowRankingGroupMy(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initView(context);
        getAttr(attrs);
    }

    public RowRankingGroupMy(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initView(context);
        getAttr(attrs, defStyleAttr);
    }

    public RowRankingGroupMy(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void initView(Context context){
        inflate(context, R.layout.row_ranking_my_group, this);
    ...
    private void getAttr(AttributeSet attrs){
        TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.RowRankingGroupMy);
        setTypedArray(typedArray);
    }
    private void getAttr(AttributeSet attrs, int defStyle){
        TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.RowRankingGroupMy, defStyle, 0);
        setTypedArray(typedArray);
    }

    private void setTypedArray(TypedArray typedArray){
        if( typedArray!=null ) {
            int bgColor = typedArray.getResourceId(R.styleable.RowRankingGroupMy_recordBg, -1);
            if (bgColor > 0) {
                mBox.setBackgroundResource(bgColor);
            }
            int textColor = typedArray.getColor(R.styleable.RowRankingGroupMy_recordTextColor, -1);
